...
type CartContextValue = {
  ...
  clear: () => void;
  /* Slide-in cart drawer (site chrome, like the menu takeover). Adding an
     item auto-opens it (openDrawer: false opts out — e.g. Buy Now, which
     routes straight to /cart and shouldn't flash the drawer first). */
  drawerOpen: boolean;
  openDrawer: () => void;
  closeDrawer: () => void;
};
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const addItem = useCallback(
    (
      ...
      // Show the item landing in the cart (premium-storefront standard) —
      // callers that immediately navigate (Buy Now) pass openDrawer: false.
