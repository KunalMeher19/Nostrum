...
- **Public page (`src/components/pages/ProductDetailPage.tsx`):** new `resolveProductCopy()` implements a three-step fallback so a half-filled product never renders blank — this locale's translation → the base English copy → the existing static locale-file defaults. Description splits on blank lines into paragraphs; details rows take their order and identity from the base copy while the translation supplies label/value overrides. The old hardcoded arrays survive untouched as the last fallback, so **products that predate this change look exactly as before**.
