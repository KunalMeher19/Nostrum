...
> Updated 2026-09-07 after completing secure guest-order claiming: customers still create and use an account immediately, while a premium Account → Details verification panel and resend flow protect the sensitive import of historical guest orders. See 2.24 and the decision log.
...
- **B2B block at the END of the Shop + Home CTA (brief §04) — PARKED by project decision 2026-08-06.** Today only a top-of-shop B2B button routing to the contact form ("professional" topic) exists. **B2B button redesigned 2026-09-02:** was a busy lime/gold aurora animation and dashed-border treatment, then a gold-frame treatment on 2026-08-28; it is now a restrained home-collection-style pill with a 1px gold outline, warm transparent rest state, centered letter-spaced label, right-aligned arrow, and a subtle hover tint on both `/products` and `/product/[id]`. The change matches the attached collection reference and keeps the Shop's discovery CTA language consistent. Verified with `npm run build`.
- **Locale entry behavior — DONE 2026-09-02:** was a year-long `NEXT_LOCALE` cookie that could reopen the site in the last visited language; it now defaults unprefixed entry to `/en`, removes the persistent cookie writer, and uses a tab-scoped `sessionStorage` guard. A visitor can switch language and keep it across every page in that tab, while a newly opened tab visiting a non-English locale is returned to the English home page. Verified by build; the repository-wide typecheck still reports two pre-existing `unknown` errors in `tests/mobile-browser-audit.spec.ts`.
...
**Frontend:**
- **Checkout review redesign — DONE 2026-08-27:** `/[locale]/checkout` now follows the client's compact reference composition: progress-led two-column review, saved shipping address summary with an explicit Edit path, inline delivery reassurance, product thumbnail in the order summary, restrained olive/cream controls, trust row, and responsive mobile stacking. Guest or incomplete-profile checkout still opens the editable address form, and the existing Stripe payload/validation flow is unchanged. Verified with `npm run build` and the changed component reports no editor diagnostics. Focused Playwright inspection was attempted but remains blocked on this machine because the required Chromium headless shell is unavailable.
...
- **Email verification — by design NOT enforced as login gate:** Registration sends a branded verify-email (now with Resend templates), but login works immediately without clicking it. The `emailVerified` timestamp is recorded when the link is clicked. **Since 2026-09-07, claiming historical guest orders is deliberately gated by that verification step;** the customer sees a resend-capable Order protection panel in Account → Details. Decision: enforcing it would lock out users whose mail hits spam — wrong trade-off for an olive oil shop. If checkout-gating on verified email is wanted later, add the check in `src/auth.ts` `authorize()` callback. **CHECK AT LAUNCH** that this decision still holds once real customers are registering.
...
Client reported that real mobile phones could carry the swipe used to reach the last `/origins` slide into the following page section and collapse the browser address bar. The mobile StoryScenes controller now tracks the transition into the final slide and consumes residual touch movement from that same gesture. The handoff gesture itself is also consumed after the final slide settles, keeping the browser chrome visible; the following downward touch releases Lenis so the page can continue. Wheel behavior now also avoids queueing a handoff from inertia while entering the final slide. Desktop ScrollTrigger behavior, upward slide navigation, and the first-slide exit remain unchanged.
...
### 2.23 Product page Description + Details editable per language — DONE 2026-09-06

Client request: the DESCRIPTION and DETAILS tabs on the single-product page were hardcoded copy with no way to change them from the admin Shop tab, and both had to become editable in all 5 locales. Was: `ProductDetailPage.tsx` read `product.desc_1/desc_2` and five fixed `product.detail_*` pairs (Variety / Extraction / Acidity / Origin / Keep) straight out of the locale JSON, identical for every product; the admin had a single plain `description` textarea whose value was stored but never rendered anywhere on the public site.

- **Schema (`backend/src/models/product.model.js`):** added `details: [{ id, label, value }]` (base/English rows, in display order) and `translations: { es|ca|it|el: { description, details } | null }`, mirroring the journal-post translations pattern already in `post.model.js`. `description` keeps its existing meaning as the base copy. **`id` is an opaque minted row identity, deliberately not derived from the label** — renaming "Variety" must not orphan the translations attached to that row.
- **Admin API (`backend/src/routes/admin.routes.js`):** new `cleanDetails()` + `cleanProductTranslations()` sanitizers wired into both `POST /api/admin/products` and `PATCH /api/admin/products/:id`. Rows without an id are dropped, duplicate ids collapse to the first, caps are 12 rows · 60-char labels · 200-char values · 2000-char descriptions, and a locale whose description and every value are blank is stored as `null` so the frontend falls straight back to English.
- **Public API (`backend/src/routes/products.routes.js`):** `GET /api/products` and `GET /api/products/featured` now return `description`, `details` and `translations`.
- **Public page (`src/components/pages/ProductDetailPage.tsx`):** new `resolveProductCopy()` resolves this locale's translation → the base English copy. Description splits on blank lines into paragraphs; details rows take their order and identity from the base copy while the translation supplies label/value overrides. **Static-copy fallback REMOVED same day on client instruction** (was: a third fallback step to the old hardcoded `product.desc_1/desc_2` + five `product.detail_*` pairs, added so existing products would not go blank on ship). The client saw the live page showing five generic Variety/Extraction/Acidity/Origin/Keep rows while the admin editor showed none, and chose the exact-mirror behaviour instead: **a tab the admin leaves empty is not rendered at all.** `visibleTabs` is now derived from what actually has content and the rendered tab is resolved against that list rather than trusting the `tab` state, so a tab disappearing under the user (locale switch, admin edit) cannot leave the panel blank. SHIPPING is genuinely shared copy and is always present.
- **Dead locale keys left in place on purpose:** `product.desc_1`, `desc_2` and the ten `product.detail_*` keys are no longer read by any component. They are kept in all 5 locale files as the reference text for the client to paste into the admin while filling the catalogue. **Delete them once the real products are entered.**
- **Admin UI (`src/components/AdminPortal/ProductCopyEditor.tsx`, new):** replaces the lone description textarea inside `ProductEditor`. One card, a language rail across the top (EN · ES · CA · IT · EL), one locale open at a time. Each pill carries a completeness dot (hollow = empty, gold = partial, lime = complete) so missing translations read at a glance without opening five tabs. English is the base tab and the only one that can add / reorder / delete detail rows; the other four project those same rows by id and only translate the wording, with **the English string shown as the placeholder of each translated field** so an untranslated box reads as "inherits this" rather than "is broken". Each non-English tab gets a "Copy from English" seed button.
- **Styling (`admin-portal.css`):** new `ad__copy*` block in the existing dark ledger material, gold marking the open language and lime marking finished ones, reusing `ad__rows`/`ad__row`/`ad__chip`/`ad__hint`. Reduced-motion safe (panel fade disabled); below 720px the rail drops to language codes only and rows stack.
- **i18n:** 18 new `admin.copy_*` keys added to all 5 locale files. No em-dash characters used.
- **Verified:** `npm run build` clean (all routes prerender); `npx tsc --noEmit` reports only the two pre-existing `fps`-is-`unknown` errors in `tests/mobile-browser-audit.spec.ts`; backend suite run by the client, green. The `[MONGOOSE] Warning: the 'new' option for findOneAndUpdate() is deprecated` line in the test output is **pre-existing and unrelated** (7 call sites across `admin.routes.js`, `me.routes.js`, `orders.service.js`; this round added none).
- **Not in scope this round:** the SHIPPING tab and the product highlights list are still locale-JSON only and identical for every product. Same pattern applies if the client wants them per-product later. This closes the "rich per-size marketing copy" schema question left open in §1.2.

### 2.24 Secure guest-order claiming + account verification UI — DONE 2026-09-07

Guest checkout remains fully available and anonymous `/api/auth/session` responses are treated as the normal logged-out state in the checkout review page. A paid guest order is stored with `userId: null`; it becomes visible in Account only after the customer proves ownership of the purchase email.

- **Frictionless account creation retained:** credentials registration sends the customer directly into their new account. Email verification is not a login or checkout gate.
- **Claiming is protected:** clicking the one-time verification link marks `emailVerified` and atomically links all guest orders with the same normalized email address. This prevents a person who merely typed another customer’s email at registration from reading their historical delivery/order data.
- **Webhook race closed:** when Stripe creates the paid order after verification, it resolves the matching verified account and attaches the order directly instead of leaving it orphaned as a guest order.
- **Premium in-account UI:** Account → Details now contains an olive-and-gold “Order protection” panel for unverified customers, explaining the benefit, with sending/sent/error states and a signed-in-only resend endpoint (`POST /api/auth/verify/resend`). The panel is localized in en/es/ca/it/el and leaves profile, shipping, marketing preferences, and order views unchanged.
- **Delivery resilience:** a temporary verification-email delivery failure cannot block account creation; the Details panel gives the signed-in customer a retry path.
- **Verified:** all locale JSON parses; `backend/__tests__/orders.test.js` passes 10/10; `git diff --check` passes. Repository `tsc --noEmit` continues to report only the pre-existing `fps`-is-`unknown` errors in `tests/mobile-browser-audit.spec.ts`. Recommended production smoke test: guest payment → create account with same email → sign in immediately → resend/receive verification email → click link → confirm the previous order appears in Account.
...
- **2026-09-07** · Historical guest orders are claimed only after email ownership is verified. The client wanted no friction at account creation, so registration still creates the account and signs the customer in immediately; verification is a quiet Account → Details action, not a blocking screen. The verification email remains the proof required to import prior guest orders, because automatically claiming by a typed email would expose another customer’s order and delivery information. A premium Order protection panel explains this purpose and offers resend. Stripe’s webhook also links an order if verification completed before webhook delivery, closing the registration/payment timing race. See 2.24.

- **2026-09-06** · Password-reset flow tightened (client request). The request now normalizes the email and performs a minimal indexed lookup in the shared `users` collection before issuing a reset token; an unknown address receives an explicit “account not found — create one first” response instead of a misleading sent-email confirmation. The account UI reads the API response, provides localized validation/rate-limit feedback, and locks the field and submit action after a successful request with a clear “Link sent” state. Reset links are now **server-enforced 15-minute, one-time tokens**: issuing another link invalidates the previous token, and successful submission atomically consumes the token via `findOneAndDelete`, so neither an expired nor an already-used link can change a password. The 15-minute/one-time limit is shown in the request UI, sent confirmation, reset page, and branded email; expired/used links now have a specific recovery message. All account copy is localized in en/es/ca/it/el. **Verified:** locale JSON parses and `git diff --check` passes; full build remains environment-blocked only by Google Fonts download access, while `tsc --noEmit` retains its two pre-existing `fps`-is-`unknown` test errors.

- **2026-09-06** · Product page copy made per-product and multilingual (client request). Design decisions taken while building it, so they are not re-litigated later: (1) **English is the base, not a peer language** — it owns the DETAILS row list and is the fallback target, because the alternative (five equal locales) means adding a row five times and offers no sane behaviour when one locale is blank. (2) **Detail rows are keyed by an opaque minted id, not by their label** — the client renames labels, and slug-derived ids would silently orphan every translation attached to the renamed row. (3) **Translators cannot add or delete rows**, only fill wording, which is what keeps the five locales structurally identical and is exactly what the client asked for ("make keys, for which we just have to put the values"). (4) **No static fallback: empty tab = hidden tab.** This was initially built the other way (the old hardcoded locale-JSON copy kept as a final fallback so nothing changed visually until the client typed something), and the client rejected it the same day on seeing the consequence: the admin editor showed no detail rows while the live page still showed five generic ones, so the editor was not a truthful preview and a product could never have zero rows. Exact-mirror behaviour won. The now-unused `product.desc_*` / `product.detail_*` keys stay in the locale files as paste-in reference text until the real catalogue is entered, then get deleted. (5) SHIPPING tab + highlights deliberately left as shared copy this round; raise with the client only if they ask. See 2.23.

- **2026-09-06** · Create-account consent row fixed (client feedback). The privacy policy link in the signup form was an `<a href="/{locale}/privacy">` pointing at a route that does not exist (legal texts live in the shared `LegalModal`, opened from the footer). It is now a `type="button"` that calls `showLegalModal("privacy", locale)` — the same modal the footer raises — with `preventDefault`/`stopPropagation` so clicking it does not toggle the wrapping checkbox label. CSS: `.ac__gdpr-link` sheds UA button chrome and inherits the old anchor's gold underline treatment, plus a lime `:focus-visible` ring; new `.ac__gdpr-req` renders a gold `*` after the link to mark the consent as required. `account.marketing_label` had "(optional)" stripped in all 5 locales (en/es/ca/it/el); the separate newsletter-modal label keeps its own wording. Same round: **"Continue with Google" had no pending state** — the click fired `signIn()` with zero feedback while the redirect was being negotiated, so the button looked dead. It now carries its own `googleBusy` state (separate from the form's `busy`, because the Google path leaves the page): disabled + `aria-busy`, the Google mark swaps for a spinning gold ring (`.ac__google-spin`, animation off under reduced motion), and the label swaps to a new `account.google_redirecting` key added in all 5 locales. If `signIn()` rejects (network/config failure, i.e. the redirect never happened) the button releases and shows `account.error_generic`. **Verified:** `npm run build` clean; `npx tsc --noEmit` reports only the two pre-existing `fps`-is-`unknown` errors in `tests/mobile-browser-audit.spec.ts`; all 5 locale files parse.
...
- **2026-08-27 follow-up** · Checkout sign-in link corrected: the "Sign in" link on the checkout page was pointing to the non-existent `/auth/signin` route. Fixed to point to `/account` (the correct account/login page). Guest checkout flow remains primary path; sign-in link is now functional for users who want to log in before completing purchase.

- **2026-08-27 follow-up** · Resend domain verification for external email delivery: investigated why order invoices are delivered to `office@nostrumoils.com` but not external addresses like `test@gmail.com`. Root cause: Resend is using `onboarding@resend.dev` sender address (sandbox mode), which only delivers to the account owner's verified domain. Solution documented for client: (1) verify `nostrumoils.com` in Resend dashboard by adding 3 DNS records, (2) update Railway env var `RESEND_FROM=Nostrum <no-reply@nostrumoils.com>`, (3) redeploy. No code changes needed—mailer already reads `RESEND_FROM` from env. Once domain is verified and env var updated, emails will deliver to any address worldwide.

- **2026-08-27 follow-up** · Checkout account-data prefill: the review page previously loaded `/api/me` with raw `fetch`, which addressed the Next.js origin instead of the backend/proxy route, so saved shipping details did not appear. It now uses the shared typed `api<Profile>()` client, matching the account portal and preserving authenticated proxy behavior. Verified with `npm run build`.

- **2026-08-27** · Stripe checkout end-to-end completion + premium checkout redesign: (1) **Stripe integration complete**: Fixed "Not a valid URL" error by converting relative product image paths to absolute URLs for Stripe sessions; fixed variable initialization order bug (`frontendUrl` used before declaration). Stripe checkout now fully operational — payment flow works from cart → Stripe hosted checkout → webhook order creation → success page. Webhook endpoint configured in Stripe dashboard: `https://nostrum-production.up.railway.app/api/stripe/webhook` listening for `checkout.session.completed`. `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, and `FRONTEND_URL` confirmed set in Railway. (2) **Amazon-style address review page**: Built new `/checkout` page between cart and Stripe payment — user confirms/enters shipping address before payment (like Amazon flow). Features: logged-in user detection with pre-filled email/name, full address form with validation, guest checkout option with sign-in link, order summary sidebar (sticky on desktop), premium white Shop theme with gold accent buttons. Orders automatically attached to user accounts when logged in. All 5 locales supported. Cart page now redirects to `/checkout` instead of directly to Stripe. (3) **Premium checkout redesign**: Rebuilt `/checkout` page from generic white form to luxury Shop aesthetic matching Nostrum brand — gradient warm-white background (#FAF8F2 → #F5F3ED) with subtle lime/gold radial overlays, gold gradient CTA buttons with lift hover effects, lime accent for form focus states with glow rings, enhanced shadows and depth on cards, section titles with lime→gold gradient underlines, larger typography with cinematic spacing, custom select dropdown arrow, premium form inputs with smooth transitions. Maintains white Shop consistency (Cart → Checkout → Success) while feeling genuinely luxury. (4) **Email sending investigation**: Confirmed Resend is configured with branded dark luxury HTML templates (order confirmation, shipping updates, newsletter, contact relay). Mailer service ready to send but requires `RESEND_API_KEY` to be set in Railway (currently console-stubbing). Once key is added, order confirmation emails will send automatically after payment. Client already has Resend account — only needs to add API key to Railway environment variables. **Verified**: Stripe payment working end-to-end, orders created in MongoDB, stock decremented, success page shows, checkout page premium redesign deployed, all builds clean.
